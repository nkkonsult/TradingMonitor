...
avantage sur le hasard). Sa mise en \oe uvre suppose deux conditions : que l'edge
...
\(0{,}95^{11} \approx 0{,}57\). Le risque de commettre \emph{au moins un}
...
théorème central limite ($n$ compris entre 978 et 8\,513 selon la
stratégie).
\end{conditions}
...
\midrule
\texttt{oracle} (témoin) & 2\,152 & $+0{,}2626$ & $5\times10^{-53}$    & \textbf{bat le hasard} \\
\midrule
...
si le edge moyen positif est dû seulement à l'échantillon, auquel cas on conserve H0 ce qui
...
D'une part, un edge positif ne signifie pas des gains positifs, mais simplement que l'on perd moins que le hasard. 
D'autre part, dû au fait que n est grand, la loi normale de la moyenne a une largeur qui s'écrase~: 
on fait alors ressortir comme significatives des stratégies dont la moyenne d'edges n'est que très 
...
%    (df = n_eff - 1), même seuil de Bonferroni 0,0045.
...
\subsection{Résultats}
% Chiffres RÉELS (base régénérée, 40 362 trades, 11 stratégies) extraits de :
...
\midrule
\texttt{oracle} (témoin) & 482 & $\approx 0$          & 188 & $\approx 0$ & \textbf{OUI} \\
\midrule
...
%  - L'ORACLE (témoin) passe les DEUX portes de très loin (p_A et p_B ~ 0) :
%    l'outil sait donc bien dire OUI quand l'avantage est réel. C'est la
%    preuve qu'il ne rejette pas tout par excès de sévérité.
%  - rsi_classic TOMBE AUX DEUX PORTES (0,059 et 0,039, loin du seuil
%    0,0045). Le t = 5,6 de l'étape 1 était de la PSEUDO-RÉPLICATION :
...
%  - Conclusion intermédiaire : à part le témoin, plus AUCUNE vraie
%    stratégie ne passe les deux portes -> l'outil DISCRIMINE.
...
%  1. [FAIT le 19/07] Tableau étape 1, encadré conditions et phrase TCL
%     remis sur la base 38 210. MAIS : relecture « chiffres périmés » de
%     TOUT le rapport encore À FAIRE avant rendu (d'autres 38 035 /
%     anciennes p-values peuvent traîner ailleurs). Ne pas oublier.
