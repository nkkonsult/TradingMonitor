...
- **Comment on le détecte (de façon chiffrée) :** on repère les **sommets** et les **creux** locaux, puis on **regroupe ceux qui tombent au même prix** (à une tolérance près, **2 %**). Un groupe d'au moins **2 touches** définit une **droite horizontale** (le niveau = le prix moyen du groupe). *(C'est différent — et plus fidèle — qu'un « canal glissant » qui donnerait une courbe ondulée.)*
