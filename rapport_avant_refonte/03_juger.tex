...
Ce chapitre traite la question~: \emph{parmi les dix stratégies d'analyse
...
Comme on l'a vu au chapitre deux, dans nos données, les titres forment des grappes.
On a également mesuré, avec la corrélation intra-titre $\hat\rho$, qui mesure
à quel point les trades d'un même titre se ressemblent.
...
où $\bar m$ désigne le nombre moyen de trades par titre, et rho 
la mesure ... obtenue au chapitre 2 (lien).
...
On refait le test de Student de l'étape 1 avec cette erreur-type corrigée, en
...
mais plutôt évaluer son impact sur le edge moyen.
...
répartition du edge, avec pour objectif de vérifier l'impact de la dépendance
sur le edge. Car si la dépendance a un impact fort, alors notre cloche sera
...
dépendance intra-titre et que l'on met en valeur son influence sur le edge
moyen.
...
sur le edge. Cependant, le fait de conserver le poids de chaque titre nous laisse
...
mois ne dépend que de leur écart et non de leur date, le mois de janvier réagis 
comme les autres mois de l'année.

On mesure alors l'impact propre de chaque décalage par l'autocorrélation
\emph{partielle} (PACF), qui retranche l'influence des mois intermédiaires~; un
...
Pour effectuer cette correction on a supposé que les mois sont tous
...
recherchée~: c'est l'\emph{absence} de rejet qui conclut à la stationnarité.
...
L'ordre retenu \(\hat p\) est le \emph{dernier} décalage significatif~; si
...
la dépendance~: elle vient du recouvrement des périodes de détention, et ne
peut donc pas porter plus loin que la durée des trades, qui dépassent
rarement six mois. Chercher au-delà reviendrait à chercher un effet sans
cause.

Cet argument reste toutefois une plausibilité, non une preuve. Nous n'en
resterons pas là~: la validation du modèle, présentée plus bas, vérifiera
qu'aucune mémoire ne subsiste au-delà de ce plafond.

% --- L'estimation des poids -------------------------------------------
\subsubsection*{Équations de Yule-Walker}

L'ordre connu, on écrit le modèle autorégressif sur la série centrée~: le
...
dépendance, confirme le verdict.
...
cours (sondages, ANOVA, séries temporelles), or ils coïncident avec les
standards de l'économétrie financière.
L'agrégation par mois, une statistique par période puis inférence
sur la série des périodes, est exactement la démarche de
Fama et MacBeth~\cite{famamacbeth1973}, standard du domaine.
La variance de long terme estimée via l'AR poursuit le même
objectif que les erreurs-types robustes à l'autocorrélation de
Newey et West~\cite{neweywest1987}, dont elle est une version paramétrique,
validée ici par le test de Box et Pierce.
...
escalade, plus aucune stratégie ne subsiste en dehors du témoin.
